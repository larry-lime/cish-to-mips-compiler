...
let insert (x : var) (v : value) (env : env) y = if y = x then v else env y
...
let rec eval_exp ((e : rexp), (pos : int)) (env : env) : int =
  ...
  | Call (f, es) -> (
      ...
      let new_env =
        zip_args (f2.args, List.map (fun e -> eval_exp e env) es, env)
      in
      try
        eval_stmt f2.body new_env;
        0
      with Done i -> i)
